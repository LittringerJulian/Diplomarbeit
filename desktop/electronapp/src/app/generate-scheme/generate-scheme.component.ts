...
export class GenerateSchemeComponent implements OnInit {
  ...
  cdkDragReleased(event, item, i) {
    ...
    this.array[i].percentagex = this.round((event.source.getFreeDragPosition().x)/this.contentWidth,2);
    this.array[i].percentagey = this.round((event.source.getFreeDragPosition().y)/this.contentHeight,2);
